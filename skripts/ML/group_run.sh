...
sbatch --partition=$partition --gres=gpu:1 --time=$time --output="${base_out}ec_annot${task}${suffix}.out" \
    ML4com_run.py \
...
sbatch --partition=$partition --gres=gpu:1 --time=$time --output="${base_out}gt_annot${task}${suffix}.out" \
    ML4com_run.py \
    -t $tries -if $inner_fold -of $outer_fold -v 0 -st $source_dir -s "annotated" -sam $source -ms $model_source \
    -b $backend -c "gpu" -n "cosine_2" -fn "${file}" -mfn "${model_file}" -a "${algorithms[@]}" -ta $task

sbatch --partition=$partition --gres=gpu:1 --time=$time --output="${base_out}gt_cos${task}${suffix}.out" \
    ML4com_run.py \
    -t $tries -if $inner_fold -of $outer_fold -v 0 -st $source_dir -s "latent" -sam $source -ms $model_source \
    -b $backend -c "gpu" -n "cosine_2" -fn "${file}" -mfn "${model_file}" -a "${algorithms[@]}" -ta $task

sbatch --partition=$partition --gres=gpu:1 --time=$time --output="${base_out}gt_maecos${task}${suffix}.out" \
    ML4com_run.py \
    -t $tries -if $inner_fold -of $outer_fold -v 0 -st $source_dir -s "latent" -sam $source -ms $model_source \
    -b $backend -c "gpu" -n "mae_cosine_2" -fn "${file}" -mfn "${model_file}" -a "${algorithms[@]}" -ta $task

sbatch --partition=$partition --gres=gpu:2 --time=$time --output="${base_out}gt_se${task}${suffix}.out" \
    ML4com_run.py \
    -t $tries -if $inner_fold -of $outer_fold -v 0 -st $source_dir -s "latent" -sam $source -ms $model_source \
    -b $backend -c "cpu" -n "se_2" -fn "${file}" -mfn "${model_file}" -a "${algorithms[@]}" -ta $task

sbatch --partition=$partition --gres=gpu:2 --time=$time --output="${base_out}gt_mae${task}${suffix}.out" \
    ML4com_run.py \
    -t $tries -if $inner_fold -of $outer_fold -v 0 -st $source_dir -s "latent" -sam $source -ms $model_source \
    -b $backend -c "cpu" -n "mae_2" -fn "${file}" -mfn "${model_file}" -a "${algorithms[@]}" -ta $task

sbatch --partition=$partition --gres=gpu:2 --time=$time --output="${base_out}gt_aecos${task}${suffix}.out" \
    ML4com_run.py \
    -t $tries -if $inner_fold -of $outer_fold -v 0 -st $source_dir -s "latent" -sam $source -ms $model_source \
    -b $backend -c "cpu" -n "ae+cosine_2" -fn "${file}" -mfn "${model_file}" -a "${algorithms[@]}" -ta $task

sbatch --partition=$partition --gres=gpu:2 --time=$time --output="${base_out}gt_mse${task}${suffix}.out" \
    ML4com_run.py \
    -t $tries -if $inner_fold -of $outer_fold -v 0 -st $source_dir -s "latent" -sam $source -ms $model_source \
    -b $backend -c "cpu" -n "mse_2" -fn "${file}" -mfn "${model_file}" -a "${algorithms[@]}" -ta $task
