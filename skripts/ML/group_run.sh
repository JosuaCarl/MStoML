...
partition="cpu3-long"
time="14-00:00:00"
declare -a algorithms=( "Decision tree" "Gaussian Naive-Bayes" "K-neighbours classifier" "Logistic Regression" \
"Neural Network (MLP) SK-learn" "Random forest" "Support-vector classifier" )
...
sbatch --partition=$partition --cpus-per-task=1 --ntasks=$workers --time=$time --output="${base_out}ec_annot${task}${suffix}.out" \
    ML4com_run.py \
...
sbatch --partition=$partition --cpus-per-task=1 --ntasks-per-node=$workers --time=$time --output="${base_out}gt_annot${task}${suffix}.out" \
    ML4com_run.py \
    -t $tries -if $inner_fold -of $outer_fold -v 0 -st $source_dir -s "annotated" -sam $source -ms $model_source \
    -b $backend -c "gpu" -n "cosine_2" -fn "${file}" -mfn "${model_file}" -a "${algorithms[@]}" -ta $task -w $workers

sbatch --partition=$partition --cpus-per-task=1 --ntasks-per-node=$workers --time=$time --output="${base_out}gt_cos${task}${suffix}.out" \
    ML4com_run.py \
    -t $tries -if $inner_fold -of $outer_fold -v 0 -st $source_dir -s "latent" -sam $source -ms $model_source \
    -b $backend -c "gpu" -n "cosine_2" -fn "${file}" -mfn "${model_file}" -a "${algorithms[@]}" -ta $task -w $workers

sbatch --partition=$partition --cpus-per-task=1 --ntasks-per-node=$workers --time=$time --output="${base_out}gt_maecos${task}${suffix}.out" \
    ML4com_run.py \
    -t $tries -if $inner_fold -of $outer_fold -v 0 -st $source_dir -s "latent" -sam $source -ms $model_source \
    -b $backend -c "gpu" -n "mae_cosine_2" -fn "${file}" -mfn "${model_file}" -a "${algorithms[@]}" -ta $task -w $workers

sbatch --partition=$partition --cpus-per-task=1 --ntasks-per-node=$workers --time=$time --output="${base_out}gt_se${task}${suffix}.out" \
    ML4com_run.py \
    -t $tries -if $inner_fold -of $outer_fold -v 0 -st $source_dir -s "latent" -sam $source -ms $model_source \
    -b $backend -c "cpu" -n "se_2" -fn "${file}" -mfn "${model_file}" -a "${algorithms[@]}" -ta $task -w $workers

sbatch --partition=$partition --cpus-per-task=1 --ntasks-per-node=$workers --time=$time --output="${base_out}gt_mae${task}${suffix}.out" \
    ML4com_run.py \
    -t $tries -if $inner_fold -of $outer_fold -v 0 -st $source_dir -s "latent" -sam $source -ms $model_source \
    -b $backend -c "cpu" -n "mae_2" -fn "${file}" -mfn "${model_file}" -a "${algorithms[@]}" -ta $task -w $workers

sbatch --partition=$partition --cpus-per-task=1 --ntasks-per-node=$workers --time=$time --output="${base_out}gt_aecos${task}${suffix}.out" \
    ML4com_run.py \
    -t $tries -if $inner_fold -of $outer_fold -v 0 -st $source_dir -s "latent" -sam $source -ms $model_source \
    -b $backend -c "cpu" -n "ae+cosine_2" -fn "${file}" -mfn "${model_file}" -a "${algorithms[@]}" -ta $task -w $workers

sbatch --partition=$partition --cpus-per-task=1 --ntasks-per-node=$workers --time=$time --output="${base_out}gt_mse${task}${suffix}.out" \
    ML4com_run.py \
    -t $tries -if $inner_fold -of $outer_fold -v 0 -st $source_dir -s "latent" -sam $source -ms $model_source \
    -b $backend -c "cpu" -n "mse_2" -fn "${file}" -mfn "${model_file}" -a "${algorithms[@]}" -ta $task -w $workers
