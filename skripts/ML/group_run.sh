...
# sbatch --partition=$partition --time=$time --output="${base_out}annot.out" ML4com_run.py -t $tries -if $inner_fold -of $outer_fold -v 0 -st $source_dir -s "annotated" -sam $source -b $backend -c "gpu" -n "cosine_2" -fn "${file}" -a "${algorithms[@]}"
